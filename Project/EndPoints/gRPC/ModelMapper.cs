    ...
    public class ModelMapper
    {
        ...
        public Abstraction.Models.Message.CreateMessageModel FromGrpc(GrpcModel_CreateMessage input)
            => new Abstraction.Models.Message.CreateMessageModel
            {
                ...
                To = input.To is null ? [] : input.To.Select(FromGrpc).ToArray(),
                From = input.From is null ? null : FromGrpc(input.From),
                Cc = input.Cc is null ? null : input.Cc.Select(FromGrpc).ToArray(),
                Bcc = input.Bcc is null ? null : input.Bcc.Select(FromGrpc).ToArray(),
                ReplyTo = input.ReplyTo is null ? null : input.ReplyTo.Select(FromGrpc).ToArray(),
                Headers = input.Headers is null ? null : input.Headers,
